...
- 🎯 Entry points to execute your program using `python -m <my_flask_project>` or `$ my_flask_project` with basic CLI argument parsing.
...
---
# my_flask_project Flask Application

Awesome my_flask_project created by RyanLilla
...
```bash
git clone https://github.com/RyanLilla/my_flask_project my_flask_project
cd my_flask_project
make install
```
...
```bash
my_flask_project create-db   # run once
my_flask_project populate-db  # run once (optional)
my_flask_project add-user -u admin -p 1234  # ads a user
my_flask_project run
```
